...
namespace System.Net.Http
{
    ...
    internal sealed class HttpConnectionPool : IDisposable
    {
        ...
        public HttpConnectionPool(HttpConnectionPoolManager poolManager, HttpConnectionKind kind, string host, int port, string sslHostName, Uri proxyUri, int maxConnections)
        {
            ...
            _http2Enabled = (_poolManager.Settings._maxHttpVersion == new Version(2, 0));
            ...
            GetHttp2ConnectionAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            {
                TimeSpan pooledConnectionLifetime = _poolManager.Settings._pooledConnectionLifetime;
                if (http2Connection.LifetimeExpired(DateTime.UtcNow.Ticks, pooledConnectionLifetime))
                {
                    // Connection expired.
            ...
            try
            {
                ...
                if (_http2Enabled)
                {
                    ...
                    if (sslStream.NegotiatedApplicationProtocol == SslApplicationProtocol.Http2)
                    {
                        ...
                        {
                            throw new HttpRequestException(String.Format(SR.net_ssl_http2_requires_tls12, sslStream.SslProtocol));
                        }
        ...
        private async ValueTask<(Socket, Stream, TransportContext, HttpResponseMessage)> ConnectAsync(HttpRequestMessage request, bool allowHttp2, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                switch (_kind)
                {
                    ...
                    case HttpConnectionKind.Https:
                    case HttpConnectionKind.ProxyConnect:
                        stream = await ConnectHelper.ConnectAsync(_host, _port, _poolManager.Settings._configureSocket, cancellationToken).ConfigureAwait(false);
                        break;

                    case HttpConnectionKind.Proxy:
                        stream = await ConnectHelper.ConnectAsync(_proxyUri.IdnHost, _proxyUri.Port, _poolManager.Settings._configureSocket, cancellationToken).ConfigureAwait(false);
                        break;
        ...
        {
            bool lifetimeExpired = connection.LifetimeExpired(DateTime.UtcNow.Ticks, _poolManager.Settings._pooledConnectionLifetime);
        ...
        private readonly struct CachedConnection : IEquatable<CachedConnection>
        {
            ...
            public CachedConnection(HttpConnection connection)
            {
                ...
                _returnedTickCount = DateTime.UtcNow.Ticks;
            }
