        ...
        private sealed class ChunkedEncodingReadStream : HttpContentReadStream
        {
            ...
            public override int Read(Span<byte> buffer)
            {
                ...
                while (true)
                {
                        ...
                        _chunkBytesRemaining >= (ulong)_connection.ReadBufferSize)
                    {
                        ...
                        if (bytesRead == 0)
                        {
                            throw new IOException(String.Format(SR.net_http_invalid_response_premature_eof_bytecount, _chunkBytesRemaining));
                        }
                ...
                try
                {
                    ...
                    switch (_state)
                    {
                        case ParsingState.ExpectChunkHeader:
                            ...
                            {
                                throw new IOException(String.Format(SR.net_http_invalid_response_chunk_header_invalid, BitConverter.ToString(currentLine.ToArray())));
                            }
            ...
            private static void ValidateChunkExtension(ReadOnlySpan<byte> lineAfterChunkSize)
            {
                ...
                for (int i = 0; i < lineAfterChunkSize.Length; i++)
                {
                    ...
                    {
                        throw new IOException(String.Format(SR.net_http_invalid_response_chunk_extension_invalid, BitConverter.ToString(lineAfterChunkSize.ToArray())));
                    }
                }
            }
